...
const AppLayout = ({ children }: AppLayoutProps) => {
    ...
    useEffect(() => {
        const getActivePage = (event: any) => {
            const { activePage, pageTitle = null } = event.detail;
            if(!activePage) return null;

            setActivePage(activePage);
            document.title = `${Config.APP_NAME} | ${pageTitle || activePage}`;
        }
    ...
    return (
        ...
        >
            ...
            {/* Body */}
            <Box flex={1} minHeight='100dvh'>{children}</Box>
...
const Content = ({ children, activePage, pageTitle }: ContentProps) => {
    ...
    useEffect(() => {
        if(window && isPageReady) {
            setTimeout(() => {
                window?.dispatchEvent(new CustomEvent('set:active-page', { detail: { activePage, pageTitle } }));
            }, isPageReady ? 0 : 1000);
        }
...
const Sidebar = ({ activePage }: SidebarProps) => {
    ...
    useEffect(( ) => {
        const activeSidebarItem = SIDEBAR_LINKS.find((item) => item.label === activePage);
    ...
    return (
        <Flex
            ...
            width={{
                base: isCollapsed ? '3rem' : '16rem',
                '3xl': isCollapsed ? '3rem' : '18rem',
            }}
            bgColor='white'
            borderRightWidth={1}
        ...
        >
            ...
            <Box>
                <Box pl={isCollapsed ? 0 : 4} textAlign={isCollapsed ? 'center' : 'left'}>
                    ...
                    <Tooltip label={`${isCollapsed ? 'Expand' : 'Collapse'} Sidebar`} placement="right">
                        <IconButton
                            ...
                            mb={6}
                            aspectRatio={1}
                            icon={<RiMenu5Line strokeWidth={1} />}
                        ...
                        ? isCollapsed
                            ...
                            : <Flex my={6} px={4} alignItems='center' width='full'>
                                <IconButton
                                    ...
                                    icon={<img src="/icons/icon-shoppers.svg" alt="Shoppers" width={16} />}
                                ...
                                <IconButton
                                    ...
                                    icon={<img src="/icons/icon-data.svg" alt="Data" width={16} />}
                ...
                >
                    {
                        SIDEBAR_LINKS.map((item, index) => item?.isDefault === sidebarDefaultView && (
                            <SidebarItem
                                key={index}
                                icon={item.icon}
                                label={item.label}
...
const TopBar = ({ activePage }: SidebarProps) => {
    ...
    const TopNavBarItem = ({ icon, label, link, onClick }: { icon: ReactElement | string, label: string, link: string, onClick?: () => void }) => {
        return (
            <Link
                to={link}
                onClick={(event: any) => {
            ...
            >
                <Button
                    ...
                    py={1}
                    colorScheme='gray'
                    opacity={activePage === label ? 1 : 0.3}
                    borderLeftWidth={3}
                    borderLeftColor={activePage === label ? 'black' : 'transparent'}
                >
                    {
                        typeof icon === 'string'
                            ? <img src={icon} alt={label} loading="eager" />
    ...
    return (
        <Flex
            display={isTopNavBarVisible ? 'flex' : 'none'}
            direction='column'
            position='fixed'
            zIndex={100}
            bgColor='white'
        ...
        >
            ...
            >
                ...
                <IconButton
                    ...
                    rounded='full'
                    aspectRatio={1}
                    icon={<RiMenu5Line strokeWidth={1} style={{ width: '3rem' }} />}
